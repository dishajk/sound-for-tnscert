...
\input{introduction}
\input{activity-vibrating-scale}
\input{activity-rubber-band-instrument}
\input{tuning-fork}
...
\input{activity-tap-desk}
\input{activity-string-telephone}

\newthought{Question:} Why can astronauts not hear each other directly on the Moon?
\input{activity-air-travel-sound}
...
\begin{figure}
  \begin{tikzpicture}
    ...
    \shadedraw [shading=radial,inner color=xkcdBlue,outer color=xkcdBlue!10] (0,0) circle (1cm);
    \node at (0,0) {\includegraphics[width=1.25cm]{drum}};
    \node at (70:3) {\includegraphics[height=0.8cm]{ear}};
  \end{tikzpicture}
...
\begin{figure}[h]
  \begin{tikzpicture}
    \node at (0,0) {\includegraphics{vibrating-string-to}};
    \draw (0,-2.5) node {String};
    \node at (1.5,2.75) {High pressure and density (Compression)};
    \draw[|-|] (0,2.5) -- (3,2.5);
  \end{tikzpicture}
  \caption{Compression is the region where the air particles are closely packed.}
  \label{fig:vibrating-string-to}
\end{figure}
...
\begin{figure}[h]
  \begin{tikzpicture}
    \node at (0,0) {\includegraphics{vibrating-string-fro}};
    \node at (1.5,2.75) {Low pressure and density (Rarefaction)};
    \draw[|-|] (0,2.5) -- (3,2.5);
    \node at (-1.5,-2.75) {Compression};
    \draw[|-|] (-3,-2.5) -- (0,-2.5);
  \end{tikzpicture}
  \caption{Rarefaction is the region where the air particles are farther apart or loosely packed.}
  \label{fig:vibrating-string-fro}
\end{figure}
...
\caption{In ocean waves, water appears to physically move up and down, forming alternating crests (peaks of positive displacement) and troughs (valleys of negative displacement).}
\end{figure}
...
A sound wave in air does not have crests and troughs like the wave we drew on paper. The wavy line used to represent how the sound wave changes is only a representation. The actual sound wave consists of particles moving back and forth and regions of compression and rarefaction travelling through the medium. 
...
\begin{tikzpicture}
    ...
    \end{tikzpicture}
%  \checkparity This is an \pageparity\ page.%
  \caption{Figure shows sound of two different frequencies. The sound represented as a blue wave has 9 cycles, while the red wave has 3 cycles in the same time. This means that the blue wave completes 3 times as many vibrations in the same time. Therefore, its frequency is 3 times that of the red wave.}\label{fig:frequency}
%   \zsavepos{pos:ppfig1}
  \setfloatalignment{b}
\end{figure}
\section{Pitch}
...
\begin{tikzpicture}
    ...
    \end{tikzpicture}
%  \checkparity This is an \pageparity\ page.%
  \caption{Figure shows sound of two different amplitudes. The sound represented as a blue wave has larger amplitude compared to the black wave.}\label{fig:amplitude}
%   \zsavepos{pos:ppfig1}
  \setfloatalignment{b}
\end{figure}
...
Almost all natural sounds (a human voice, falling rain, or a musical instrument) are a complex mixture of multiple sound waves with different frequencies, amplitudes, and wavelengths traveling together through the air.
\begin{figure}
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption{The natural sound is a mixture of sound waves with various frequency and wavelengths.}
\end{figure}
...
\section{How Humans Hear and Process Sound}
The human ear converts sound waves into electrical signals for the brain. The visible outer part (pinna) channels sound waves into the ear canal. Sound waves hit a thin membrane called the eardrum (tympanic membrane), causing it to vibrate. These vibrations pass to three tiny bones called ossicles—the malleus (hammer), incus (anvil), and stapes (stirrup). These bones amplify the vibrations. The amplified vibrations pass into the cochlea, a snail-shaped tube filled with liquid. Liquid ripples move thousands of microscopic hair cells. These hair cells convert mechanical vibrations into electrical signals. The auditory nerve carries electrical signals to the brain, which interprets them as speech or music.
\begin{marginfigure}
...
\begin{figure}
  \begin{tikzpicture}
    \node at (0,0) {\includegraphics[width=10cm]{vibrating-string}};
